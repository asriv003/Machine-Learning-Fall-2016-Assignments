function l3_h20(test,numrep,B,X,Y)
...
%Layers = 3, Hidden units = 20
    ...
    for l = 1:3
        ...
        W_2_final = zeros(20,21);
        W_3_final = zeros(1,21);
        for n = 1:numrep
            ...
            W_1 = [rand(20,1),(rand(20,2).*2 -1)];  %20x3

            %bias + weights at 2nd layer with 1 hidden unit
            W_2_20 = [rand(20,1),(rand(20,20).*2 -1)];    %20x21

            %bias + weights at 3rd layer with 1 hidden unit
            W_3_20 = [rand,(rand(1,20).*2 -1)];    %1x21
            ...
            [H,A_3_20,A_2_20] = forward_prop_with_3_layer(A_1,W_1,W_2_20,W_3_20);

            total_loss_old = compute_loss_function_with_3_layer(Y,H,W_1,W_2_20,W_3_20,lambda(l));

            while abs(total_loss_old - total_loss_new) > 1e-8*abs(total_loss_old)
                %error at output layer
                total_loss_old = compute_loss_function_with_3_layer(Y,H,W_1,W_2_20,W_3_20,lambda(l));
                D_4 = H-Y;
                [D_3_20,D_2_20] = backward_prop_with_3_layer(D_4,A_3_20,W_3_20,A_2_20,W_2_20);
                ...
                tA_3_20 = A_3_20;
                %gradient descent
                [W_1,W_2_20,W_3_20] = gradient_descent_with_3_layer(A_1,A_2_20,A_3_20,D_2_20,D_3_20,D_4,W_1,W_2_20,W_3_20,eta,lambda(l));
                %forward propagation with new weights
                [H,A_3_20,A_2_20] = forward_prop_with_3_layer(A_1,W_1,W_2_20,W_3_20);

                %calculate loss function with the change in weights
                total_loss_new = compute_loss_function_with_3_layer(Y,H,W_1,W_2_20,W_3_20,lambda(l));
            ...
            if(n == 1)
                ...
                W_2_final = W_2_20;
                W_3_final = W_3_20;
                total_loss_final = total_loss_old;
            else
                if(total_loss_old < total_loss_final)
                    W_1_final = W_1;
                    W_2_final = W_2_20;
                    W_3_final = W_3_20;
                    total_loss_final = total_loss_old;
        ...
        if(lambda(l) == 0.001)
            title('layer = 3, hidden units = 20, \lambda = 0.001');
        elseif(lambda(l) == 0.01)
            title('layer = 3, hidden units = 20, \lambda = 0.01');
        elseif(lambda(l) == 0.1)
            title('layer = 3, hidden units = 20, \lambda = 0.1');
        end
    end
end
...
function [D_3, D_2] = backward_prop_with_3_layer(D_4,A_3,W_3,A_2,W_2)
    ...
    D_2 = (A_2.*(1-A_2)).*(D_3(:,2:21)*W_2);
end

function [W_1, W_2, W_3] = gradient_descent_with_3_layer(A_1,A_2,A_3,D_2,D_3,D_4,W_1,W_2,W_3,eta,lambda)
    %New weight values
    W_1 = W_1 - eta.*(D_2(:,2:21)'*A_1) - lambda.*eta.*W_1;
    W_2 = W_2 - eta.*(D_3(:,2:21)'*A_2) - lambda.*eta.*W_2;
